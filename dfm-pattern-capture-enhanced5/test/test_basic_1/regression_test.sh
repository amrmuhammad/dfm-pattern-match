...
# Drops database, runs generate_test_gds, runs dfm_pattern_capture,
...
cd "${TEST_DIR}"
${BUILD_DIR}/generate_test_gds || {
    echo "Error: generate_test_gds failed"
    exit 1
}

# Step 3: Run dfm_pattern_capture
echo "Running dfm_pattern_capture"
cd "${TEST_DIR}"
COMMAND="${BUILD_DIR}/dfm_pattern_capture --layout_file ./test.gds --mask_layer_number 66 --mask_layer_datatype 20 --input_layers \"67:20,68:20,69:20\" --db_name ${DB_NAME}"
